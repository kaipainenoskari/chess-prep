...
export interface UseAnalyzePositionResult {
  ...
  error: string | null;
  loading: boolean;
  submit: (fen: string, options?: LineAnalysisOptions) => Promise<boolean>;
  reset: () => void;
}
...
export function useAnalyzePosition(): UseAnalyzePositionResult {
  ...
  const submit = useCallback(
    async (fen: string, options?: LineAnalysisOptions): Promise<boolean> => {
      ...
      setLoading(true);
      try {
        const body: { fen: string; options?: LineAnalysisOptions } = {
          fen: v.data,
        };
